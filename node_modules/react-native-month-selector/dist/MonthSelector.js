...
class MonthSelector extends React.Component {
    ...
    getMonthComponent(month) {
        if (this.isMonthEnabled(month)) {
            return (<TouchableOpacity onPress={() => this.handleMonthTaps(month)} style={{ flex: 1, alignItems: "center" }}>
    ...
    renderQ(months, qNo) {
        ...
        return (<View style={[styles.horizontalFlexViewStyle]}>
    ...
    renderHeader() {
        ...
        <TouchableOpacity onPress={() => this.handNextPrevTaps(false)}>
          {this.props.prevIcon ? (this.props.prevIcon) : (<Text>{this.props.prevText}</Text>)}
        </TouchableOpacity>
        ...
        <TouchableOpacity onPress={() => this.handNextPrevTaps(true)}>
          {this.props.nextIcon ? (this.props.nextIcon) : (<Text>{this.props.nextText}</Text>)}
        </TouchableOpacity>
      </View>);
    }
    ...
    render() {
        ...
        const { containerStyle, swipable, velocityThreshold, directionalOffsetThreshold, gestureIsClickThreshold, } = this.props;
        const SWIPE_CONFIG = {
            velocityThreshold,
            directionalOffsetThreshold,
            gestureIsClickThreshold,
        };
        return (<GestureRecognizer onSwipe={direction => (swipable ? this.handleSwipe(direction) : null)} config={SWIPE_CONFIG} style={[styles.container, containerStyle]}>
        {this.renderHeader()}
